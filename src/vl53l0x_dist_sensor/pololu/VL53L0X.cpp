...
// Should be 8 bit address with lsb present
#define ADDRESS_DEFAULT 0x52

// Record the current time to check an upcoming timeout against
#define startTimeout() (timeout_start_ms = time_service::getCurTime())

// Check if timeout is enabled (set to nonzero value) and has expired
#define checkTimeoutExpired() (io_timeout > 0 && (time_service::getCurTime() - timeout_start_ms) > io_timeout)
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
    ...
    auto val_high = receiveByte();
    ack();

    auto val_low = receiveByte();
    ack();

    stop();

    return common_functions::bytesToUint16( val_low, val_high );
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
    ...
    auto val_4 = receiveByte();
    ack();

    auto val_3 = receiveByte();
    ack();

    auto val_2 = receiveByte();
    ack();

    auto val_1 = receiveByte();
    ack();

    stop();

    return common_functions::bytesToUint32( val_1, val_2, val_3, val_4 );
}
